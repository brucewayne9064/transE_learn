...
def data_loader(file):
    ...
    with codecs.open(file2, 'r') as f1, codecs.open(file3, 'r') as f2:  # 打开entity2id和relation2id
        lines1 = f1.readlines()  # readlines：作为列表返回文件中的所有行，其中每一行都是列表对象中的一项
        lines2 = f2.readlines()  # 也就是将每一行拿出来组成一个列表返回

        for line in lines1:
            line = line.strip().split('\t')  # 这一步是拆分出实体和编号    strip()：删除头尾的空格     \t：水平制表符相当于tab   split把‘符号’左右的元素拆开
            if len(line) != 2:  # 如果这一行不是两个元素，就放弃这一行
                continue
            entity2id[line[0]] = int(line[1])  # 转换成字典{实体：id}   字典名称叫entity2id  每个元素类似‘/m/06rf7’={int}0
    ...
    with codecs.open(file1, 'r') as f:  # 文件读尽量用codecs.open方法，一般不会出现编码的问题
        content = f.readlines()  # 读取训练集 train
        for line in content:
...
class TransE:
    ...
    def emb_initialize(self):
        ...
        for entity in self.entity:
            ...
            e_emb_temp = np.random.uniform(-6 / math.sqrt(self.embedding_dim),
                                           6 / math.sqrt(self.embedding_dim),
                                           self.embedding_dim)
            entity_dict[entity] = e_emb_temp / np.linalg.norm(e_emb_temp, ord=2)  # 实体也做了l2归一化，但是原伪代码没有

        self.relation = relation_dict  # 初始化后的关系  {关系编号：L2范数归一化后的向量}
        self.entity = entity_dict  # 初始化后的实体   {实体编号：L2范数归一化后的向量}

    def train(self, epochs):  # 当一个完整的数据集通过神经网络一次并且返回一次的过程称为一个epoch
        nbatches = 400  # 根据batch size计算的，看有多少个batch
        batch_size = len(self.triple_list) // nbatches  # 根据三元组数量来判断batch_size
        print("batch size: ", batch_size)  # 在不能将数据一次性通过神经网络的适合，就需要将数据集分成几个batch, batch_size:一个batch中的样本总数（一次训练所选取的样本数）
        for epoch in range(epochs):
            ...
            for k in range(nbatches):
                ...
                Sbatch = random.sample(self.triple_list, batch_size)  # 取一个batch数量的三元组样本
                Tbatch = []  # 负样本
                num = num + 1
                # 每个triple选3个负样例
                # for i in range(3):
    ...
    def update_embeddings(self, Tbatch):
        ...
        for triple, corrupted_triple in Tbatch:
            ...
            # 取原始的vector计算梯度，triple里是索引，entity是字典，取到原始向量
...
if __name__ == '__main__':
    ...
    print("Complete loading. entity : %d , relation : %d , triple : %d" % (
        len(entity_set), len(relation_set), len(triple_list)))
    #   Experimental setup：k = 50(embedding_dim), learning_rate = 0.01, margin = 1, and d = L1(L1范数，曼哈顿距离)
